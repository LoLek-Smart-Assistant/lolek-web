...
export function MatchHistoryPanel({
      ...
      <section className="rounded-[30px] border border-white/10 bg-slate-950/75 p-5 shadow-[0_20px_70px_rgba(8,15,35,0.35)]">
        ...
        <div className="mt-5 space-y-3">
          {Array.from({ length: 4 }).map((_, index) => (
            <SkeletonCard key={index} className="h-28 rounded-[22px]" />
          ))}
        </div>
      </section>
    )
  ...
  return (
    <section className="rounded-[30px] border border-white/10 bg-slate-950/75 p-5 shadow-[0_20px_70px_rgba(8,15,35,0.35)]">
      ...
      {entries.length === 0 ? (
        <div className="mt-5 rounded-[22px] border border-dashed border-white/10 bg-white/[0.03] px-4 py-10 text-center text-sm text-slate-400">
